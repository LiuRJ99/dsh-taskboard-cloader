...
const stale = (await (await fetch(`${base}/agent-taskboard/state`)).json()).value.tasks
  .filter(t => /P3 执行链路验证|P4 双向协作验证|prepare 排查|Dev-server verification/.test(t.title))
for (const t of stale) {
  if (t.trashedAt === undefined) await post(`/agent-taskboard/tasks/${t.id}/delete`, {})
  await post(`/agent-taskboard/tasks/${t.id}/delete`, { purge: true })
}
