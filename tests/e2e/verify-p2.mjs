// Real-environment verification of the /dsh-taskboard routes on the dev server.
...
if (first) {
  ...
  if (id) {
    const mv = async (ifVersion, status) => post(`/dsh-taskboard/tasks/${id}/move`, { ifVersion, status })
...
try {
  const ledger = JSON.parse(readFileSync(`${homedir()}/.dsh/dsh-taskboard.json`, 'utf8'))
