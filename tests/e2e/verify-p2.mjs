// Real-environment verification of the /agent-taskboard routes on the dev server.
...
if (first) {
  ...
  if (id) {
    const mv = async (ifVersion, status) => post(`/agent-taskboard/tasks/${id}/move`, { ifVersion, status })
...
try {
  const ledger = JSON.parse(readFileSync(`${homedir()}/.dsh/agent-taskboard.json`, 'utf8'))
