...
describe('taskboard routes', () => {
  ...
  it('persists reasoning effort, speed, and permission across create/update', async () => {
    ...
    expect(created.json.value.permission).toBe('danger-full-access')
    ...
    expect(updated.json.value.permission).toBe('read-only')
    ...
    const badPermission = await post('/dsh-taskboard/tasks', { title: 'Bad permission', workspaceId: 'ws-a', urgency: 'normal', permission: 'full-access' })
    expect(badPermission.status).toBe(400)

    // Legacy fork spelling `permissionMode` is accepted and canonicalized.
    const legacy = await post('/dsh-taskboard/tasks', {
      title: 'Legacy permissionMode',
      workspaceId: 'ws-a',
      urgency: 'normal',
      permissionMode: 'read-only',
    })
    expect(legacy.status).toBe(201)
    expect(legacy.json.value.permission).toBe('read-only')
  })
...
describe('taskboard routes 0.4.0 (checklist / templates / import / diff)', () => {
  ...
  it('templates: seeds built-ins, upserts, renames, deletes', async () => {
    ...
    const builtins = list.value.templates as Array<{ name: string; task: { speed?: string; permission?: string } }>
    expect(builtins.find(t => t.name === 'Bug 修复')?.task).toMatchObject({ speed: 'standard', permission: 'workspace-write' })
    expect(builtins.find(t => t.name === '发布检查')?.task).toMatchObject({ speed: 'standard', permission: 'workspace-write' })
    expect(builtins.find(t => t.name === '例行巡检')?.task).toMatchObject({ speed: 'standard', permission: 'read-only' })
    ...
    expect(created.json.value.task.permission).toBe('workspace-write')
