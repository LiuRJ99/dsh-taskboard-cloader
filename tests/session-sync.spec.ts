...
describe('ExternalSessionSyncService (0.5.4)', () => {
  ...
  async function createHarness(settings?: { syncExternalSessions?: boolean }) {
    ...
    const emit = async (sessionId: string, event: { type: string; data?: unknown }, sessionMeta?: { header?: { cwd?: string } }) => {
      ...
      // The service settles asynchronously (`void handleSessionEvent`). Poll
      // until the ledger revision stops advancing (a quiet window means the
      // handler chain drained) — a fixed 20ms sleep races under parallel-suite
      // CPU contention.
      const deadline = Date.now() + 2_000
      let lastRevision = store.snapshot().revision
      let quietMs = 0
      for (;;) {
        ...
        if (revision === lastRevision) {
          quietMs += 5
          if (quietMs >= 30) break // no ledger movement for 30ms → settled
        } else {
