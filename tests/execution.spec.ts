...
describe('ExecutionService', () => {
  ...
  it.each([
  ...
  ] as const)('applies %s permission mode and the task model options before the first prompt', async (permission, policy) => {
    ...
    const result = await svc.run(`t-${permission}`, 'manual')
    expect(result.ok).toBe(true)
    if (!result.ok) return
    ...
    expect(agents.sessionEvents).toEqual([
      { sessionId: result.sessionId, type: 'sandbox/mode', data: { mode: permission } },
    ...
    expect(framing).toContain(`权限模式：${permission}`)
  })
