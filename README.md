...
- **图片附件（0.6.9）**：任务描述与评论可选择、粘贴或拖放 PNG/JPEG/GIF/WebP，自动插入 Markdown；详情中显示缩略图，点击灯箱放大。图片保存在本机 DSH 主目录，单文件上限 5 MiB
...
- **Worktree 隔离是协作约定而非沙箱**：执行会话拥有完整工具权限，隔离依赖分支约定，不适用于运行不可信代码的场景。
- **数据本地**：台账、模板和图片附件全部存本机 DSH 主目录；不外发任何数据，无需 Token / API Key。
...
随时可顶栏「⬇ JSON」备份台账、「⬇ CSV」导出任务清单（带 BOM，Excel 直接打开）。图片附件不在 JSON 内；完整备份需同时复制 `dsh-taskboard-assets/` 文件夹。
...
- **任务描述与评论支持插入图片（[#25](https://github.com/cloader/dsh-taskboard/issues/25)）**：支持选择、粘贴和拖放 PNG/JPEG/GIF/WebP，自动插入 Markdown，并在详情中显示缩略图与灯箱预览。
- 图片按内容哈希在本地去重保存，限制格式、路径、单文件大小与总容量；台账和 SSE 只记录短链接。DSH 开发依赖升级到 0.1.5-rc.2 系列。
