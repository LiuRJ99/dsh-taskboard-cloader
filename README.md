...
- **archived 任务全面只读**：归档卡现在在 GUI 与 agent 工具两侧都不可再编辑或追加评论（此前仅部分入口受限）
- **新建任务初始状态收紧**：创建任务只能直接落在 backlog 或 todo；想进入 in_progress 请正常认领（此前允许直建任意非终态，会产生无持有者的进行中任务）
- **取消已结束的执行会得到明确报错**：对已成功/失败/取消的执行点「停止」，接口返回明确失败而非假成功
- **执行报告可补交**：持有任务的会话，或其名下最近一次执行已成功的会话，可在事后提交/覆盖该次执行的报告（此前仅运行中的执行可交）
