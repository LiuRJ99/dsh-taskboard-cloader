...
- **Fix: the second click on the session-header 看板 button opened the bottom workbench instead of collapsing the right sidebar**: `openTaskFromHeader` judged "is the right column already showing the board?" through dsh-better-sidebar 0.18.x DOM markers — `[data-dsh-panel]` (the self-drawn right panel back then) plus the `panelHidden` class, with `[data-dsh-panel-host] button[aria-label*="折叠"]` as the collapse-control fallback. Once 0.19.0 handed the right column to DSH's native Sidebar, `data-dsh-panel` was left on the plugin's own **bottom workbench** (which also carries `data-dsh-bottom-panel`) whose close button is literally labelled 折叠底部面板; and because the 0.19 build emits `..._bottomPanelHidden` (capital `P`), `includes('panelHidden')` never matched, so "the right panel is open" read as always true. Together they made the second click hit the bottom workbench's own toggle — open, then close — while the right sidebar never collapsed. The fix reads DSH's native right Sidebar controller (`ctx.sidebarRight`) first: when it `isExpanded()` and its `active()` tab is the board, `toggleExpanded()` hides the column; every other state reveals the board tab. The legacy companion path (< 0.19, which still draws the right column itself) keeps a DOM fallback scoped to a `[data-dsh-panel]` **without** `data-dsh-bottom-panel` and to the `[data-dsh-toggle-cluster]`'s last button, so it can no longer reach the bottom workbench; new `tests/right-sidebar.spec.ts` plus header-click regressions (0.19 bottom-workbench trap, legacy control selection, open→hide→open)
