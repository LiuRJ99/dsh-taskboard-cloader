/**
 * Client-side localization of built-in task templates. Built-in templates are
 * seeded host-side with zh fallback text; this module resolves the active
 * locale's content (name + task spec) so the new-task dropdown, the template
 * manager, and the create-form prefill follow the GUI language live. Custom
 * templates (no `builtin` flag) and unknown built-in ids pass through.
 *
