import { normalizeTemplateCategory } from "../shared/protocol.js";
...
const BUILTIN_TEMPLATES = BUILTIN_TEMPLATE_IDS.map((id) => {
	...
	return {
		id,
		name: content.name,
		...content.category !== void 0 ? { category: content.category } : {},
		task: { ...content.task }
	};
});
/** Mint a template id. */
...
function sanitizeLoadedTemplate(template) {
	let category;
	try {
...
var TemplateStore = class {
	...
	async ensure() {
		...
		try {
			...
			if (Array.isArray(value.templates)) parsed = value.templates.filter((t) => typeof t === "object" && t !== null && typeof t.id === "string" && typeof t.name === "string" && typeof t.task === "object").map(sanitizeLoadedTemplate);
		} catch {}
		if (parsed === void 0) {
			const now = Date.now();
			...
			parsed = parsed.map((template) => {
				const seed = template.builtin === true ? seeds.get(template.id) : void 0;
				if (seed === void 0) return template;
				const legacyPermission = template.task.permissionMode;
				const missingCategory = template.category === void 0 && seed.category !== void 0;
				const missingSpeed = template.task.speed === void 0 && seed.task.speed !== void 0;
				const missingPermission = template.task.permission === void 0 && seed.task.permission !== void 0 && legacyPermission === void 0;
				...
				const legacy = template.task.permissionMode;
				if (legacy !== void 0 && template.task.permission === void 0) {
					const { permissionMode: _dropped, ...rest } = template.task;
					return {
						...template,
						task: {
							...rest,
