import { asBoardSettings, asPermission, emptyLedger, isPlausibleTaskRecord, normalizeRequiredCapabilities, pruneExecutions } from "../shared/protocol.js";
		...
		try {
			...
			if (typeof parsed.revision === "number" && Array.isArray(parsed.tasks)) {
				...
				for (const entry of parsed.tasks) {
					...
					const legacy = entry.permissionMode;
					if (task.permission === void 0 && typeof legacy === "string") try {
