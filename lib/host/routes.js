import { asBoardSettings, asIsolation, asPermissionMode, asStatus, asTaskSpeed, asUrgency, canTransition, checklistFromTexts, defaultIsolationOf, newCommentId, newTaskId, normalizeBody, normalizeChecklist, normalizeExecution, normalizeModel, normalizePrompt, normalizeRequiredCapabilities, normalizeTemplateCategory, normalizeTitle, summarize, syncClaim, validateLedgerImport } from "../shared/protocol.js";
...
function normalizeTemplateSpec(raw, now) {
	...
	if (permissionMode !== void 0) spec.permissionMode = asPermissionMode(permissionMode);
		...
		try {
			...
			if (pathname === `/dsh-taskboard/tasks`) {
				try {
					...
					const permissionMode = body.permissionMode === void 0 ? void 0 : asPermissionMode(str(body, "permissionMode") ?? "");
				...
				try {
					...
					if (action === "update") {
						...
						await store.mutate("task-updated", (ledger) => {
							...
							else if (body.permissionMode !== void 0) next.permissionMode = asPermissionMode(str(body, "permissionMode") ?? "");
							const isolationRaw = str(body, "isolation");
