	...
	const gitAvailable = async (path) => {
		...
		if (value && !gitHinted.has(path)) {
			...
			try {
				...
				if (!(await readFile(join(path, ".gitignore"), "utf8")).split("\n").some((l) => l.trim() === ".dsh-worktrees" || l.trim() === `/.dsh-worktrees`)) console.info(`[dsh-taskboard] 建议在 ${path}/.gitignore 加入一行 ${WORKTREE_DIR}/ 以隐藏任务 worktree 目录（不会自动修改）`);
			} catch {}
		}
		return value;
	};
