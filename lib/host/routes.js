import { asBoardSettings, asIsolation, asPermission, asStatus, asTaskSpeed, asUrgency, canTransition, checklistFromTexts, defaultIsolationOf, defaultPermissionOf, isValidRelRepoPath, newCommentId, newTaskId, normalizeBody, normalizeChecklist, normalizeExecution, normalizeModel, normalizePrompt, normalizeRequiredCapabilities, normalizeTemplateCategory, normalizeTitle, summarize, syncClaim, validateLedgerImport } from "../shared/protocol.js";
...
import { registerMermaidBundleRoute } from "./mermaid-route.js";
...
function normalizeTemplateSpec(raw, now) {
	...
	if (speed !== void 0) spec.speed = asTaskSpeed(speed);
	...
	else if (permissionMode !== void 0 && permissionMode.trim().length > 0) spec.permission = asPermission(permissionMode);
	...
	if (e.requiredCapabilities !== void 0) spec.requiredCapabilities = normalizeRequiredCapabilities(e.requiredCapabilities);
		...
		try {
			...
			if (req.method === "GET") {
				...
				if (pathname === `/dsh-taskboard/prompt-completions`) {
					...
					const completions = await options.promptCompletions?.({ workspaceId }).catch(() => void 0);
					json(res, {
						ok: true,
						value: {
			...
			if (pathname === `/dsh-taskboard/tasks`) {
				try {
					...
					const speed = body.speed === void 0 ? void 0 : asTaskSpeed(str(body, "speed") ?? "");
				...
				try {
					...
					if (action === "update") {
						...
						await store.mutate("task-updated", (ledger) => {
							...
							if (body.requiredCapabilities !== void 0) next.requiredCapabilities = normalizeRequiredCapabilities(body.requiredCapabilities === null ? void 0 : body.requiredCapabilities);
							...
							else if (body.speed !== void 0) next.speed = asTaskSpeed(str(body, "speed") ?? "");
							const isolationRaw = str(body, "isolation");
							...
							else if (body.permissionMode !== void 0) next.permission = asPermission(body.permissionMode);
