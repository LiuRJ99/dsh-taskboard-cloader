import { TASKBOARD_CAPABILITY, asIsolation, asPermission, asStatus, asTaskSpeed, asUrgency, canTransition, checklistFromTexts, defaultIsolationOf, effectivePrompt, isClaim, isClaimedBy, newCommentId, newTaskId, normalizeBody, normalizeExecution, normalizeExecutionReport, normalizeModel, normalizePrompt, normalizeTitle, requiredCapabilitiesOf, summarize, syncClaim } from "../shared/protocol.js";
...
function taskLine(t) {
	...
	if (t.permission !== void 0) parts.push(`·权限 ${t.permission}`);
...
function taskDetail(t) {
	const lines = [
		...
		`隔离: ${t.isolation === "none" ? "关闭（原目录执行）" : "Git Worktree"}${t.branch !== void 0 ? `（分支 ${t.branch}）` : ""}${t.branches !== void 0 ? `（多仓库镜像 ${Object.keys(t.branches).length + (t.branch !== void 0 ? 1 : 0)} 个仓库）` : ""}`
	];
	...
	if (t.speed !== void 0) lines.push(`速度: ${t.speed === "fast" ? "快速" : "标准"}`);
	if (t.permission !== void 0) lines.push(`权限模式: ${t.permission}`);
...
function registerTaskboardTools(ctx, deps) {
	...
	disposers.push(register(defineTool({
		...
		description: "Create a task on the board. Required: title, workspaceId (project), urgency (urgent/normal/relaxed). Optional: description, prompt (sent to a fresh session on execution), status (default todo), execution mode (claim|scheduled + cron), model {provider, model, reasoningEffort?} to pin executions to a model, speed (standard|fast), and permission (read-only|workspace-write|danger-full-access; legacy alias permissionMode is also accepted). Do not track trivial requests as tasks.",
		parameters: {
			title: {
				type: "string",
				required: true,
			...
			model: {
				...
				properties: {
					...
					reasoningEffort: {
						type: "string",
						description: "Optional adapter-owned thinking intensity / reasoning effort (e.g. low, medium, high); omit for the model/provider default."
					}
				}
			},
			speed: {
		...
		async execute(args, exec) {
			try {
				...
				const speed = args.speed === void 0 ? void 0 : asTaskSpeed(args.speed);
				const permission = args.permission === void 0 ? args.permissionMode === void 0 ? void 0 : asPermission(args.permissionMode) : asPermission(args.permission);
		...
		async execute(args, exec) {
			try {
				...
				await store.mutate("task-moved", (ledger) => {
					...
					syncClaim(next, to, deps.now(), claimed ? actor.sessionId : void 0);
					ledger.tasks[index] = next;
					return [next];
				});
				if (claimed && deps.authorizeSession !== void 0) try {
					...
					try {
						await store.mutate("task-moved", (ledger) => {
							...
							if (current === void 0 || current.status !== "in_progress" || current.claimedBy !== actor.sessionId || current.version !== next.version) throw new Error("claim changed before capability authorization could be rolled back");
