import { approvalPolicyForPermissionMode, effectiveIsolation, effectivePrompt, effectiveTaskSpeed, newCommentId, newExecutionId, normalizeBody, requiredCapabilitiesOf } from "../shared/protocol.js";
import { sanitizeBranchName, worktreePathOf } from "./git.js";
import { MessageId } from "./sdk.js";
import { serviceTierForTaskSpeed } from "../shared/model-capabilities.js";
...
function appendPermissionMode(session, mode) {
	if (typeof session !== "object" || session === null || typeof session.append !== "function") throw new Error("permission mode unavailable: execution session has no append face");
...
var ExecutionService = class {
	...
	*/
	...
	async resolveServiceTier(model, speed) {
		if (model === void 0 || speed !== "fast" || this.deps.modelCapabilities === void 0) return void 0;
		try {
	...
	*/
	async run(taskId, trigger, options) {
		...
		const speed = serviceTier === "priority" ? "fast" : "standard";
		let composition;
		try {
		...
		let handle;
		try {
			const needsModelOptions = model !== void 0 || serviceTier !== void 0;
			const setup = !needsModelOptions && composition === void 0 ? void 0 : async (agentCtx) => {
		...
		try {
			if (model !== void 0) await this.deps.modelExecution?.(sessionId, model, speed);
		} catch {}
		try {
			if (task.permissionMode !== void 0) await (this.deps.applyPermissionMode ?? appendPermissionMode)(handle.agent.session, task.permissionMode);
		} catch (error) {
			const message = error instanceof Error ? error.message : String(error);
			try {
		...
		try {
			...
			await this.patchExecution(executionId, {
				outcome: "failed",
				error: `能力授权失败：${message.slice(0, 400)}`,
				endedAt: this.deps.now()
			});
			await this.revertProgress(taskId);
			if (prepared !== void 0 && this.deps.git !== void 0) try {
			...
			return {
				ok: false,
				error: `capability authorization failed: ${message}`
			};
		}
		...
		handle.agent.inject({
			...
			content: [{
				type: "text",
				text: this.pluginFraming(task, prepared, isolationNote, speed)
			}],
			source: {
				kind: "plugin",
