...
var ExecutionService = class {
	...
	*/
	async run(taskId, trigger, options) {
		...
		try {
			if (task.permission !== void 0) await (this.deps.applyPermissionMode ?? appendPermissionMode)(handle.agent.session, task.permission);
		} catch (error) {
			const message = error instanceof Error ? error.message : String(error);
			try {
