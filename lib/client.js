...
const TASKBOARD_CAPABILITY = "taskboard";
...
*/
function normalizeRequiredCapabilities(raw) {
	if (raw === void 0) return [TASKBOARD_CAPABILITY];
	if (!Array.isArray(raw)) throw new Error("requiredCapabilities must be an array of skill names");
	if (raw.length > 20) throw new Error(`requiredCapabilities may hold at most ${20} skills`);
	const seen = /* @__PURE__ */ new Set();
	const result = [TASKBOARD_CAPABILITY];
	...
	for (const value of raw) {
		...
		if (!TASK_CAPABILITY_RE.test(name)) throw new Error(`invalid required capability "${name}"`);
...
var BoardController = class {
	...
	*/
	...
	installCurrentSessionAuthorizer(authorizer) {
		this.currentSessionAuthorizer = authorizer;
	}
	/** Best-effort user-panel grant; task creation itself remains usable on old hosts. */
	async authorizeCurrentSession(sessionId) {
		if (this.currentSessionAuthorizer === void 0) return false;
		try {
	...
	*/
	...
	async createFromPanel(body, sessionId) {
		const id = await this.create(body);
		if (id !== void 0) await this.authorizeCurrentSession(sessionId);
		return id;
	}
...
/* The shell's collapsed rail has no room for text; keep the taskboard as an icon button. */
.hHd-Xa_collapsed .dsh-atb-entry,
[data-sidebar-collapsed] .dsh-atb-entry {
  width: 36px; height: 36px; margin: 2px auto; padding: 0; justify-content: center; gap: 0;
}
.hHd-Xa_collapsed .dsh-atb-entry-label,
.hHd-Xa_collapsed .dsh-atb-entry-stats,
[data-sidebar-collapsed] .dsh-atb-entry-label,
...
.dsh-atb-detail-edit[data-active="true"] {
  border-color: var(--dsw-alias-brand-primary, #8e4ec6);
...
.dsh-atb-promptbox.dsh-atb-raw {
  font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace;
}
.dsh-atb-desc { word-break: break-word; font-size: 13px; line-height: 1.55; }

/* ---------- markdown display (0.6.0, display-only and locally scoped) ---------- */
.dsh-atb-md { min-width: 0; max-width: 100%; word-break: break-word; }
/* Source mode is rendered as escaped React text, preserving Markdown markers
   and line breaks without entering the dangerouslySetInnerHTML path. */
.dsh-atb-raw {
  min-width: 0; max-width: 100%; white-space: pre-wrap;
  word-break: break-word; overflow-wrap: anywhere;
}
.dsh-atb-md p { margin: 0 0 .65em; word-break: break-word; overflow-wrap: anywhere; }
.dsh-atb-md p:last-child { margin-bottom: 0; }
.dsh-atb-md h1, .dsh-atb-md h2, .dsh-atb-md h3, .dsh-atb-md h4 {
  margin: .7em 0 .35em; line-height: 1.3; color: var(--dsw-alias-label-primary, inherit); word-break: break-word;
}
.dsh-atb-md h1 { font-size: 1.35em; }
.dsh-atb-md h2 { font-size: 1.2em; }
.dsh-atb-md h3 { font-size: 1.08em; }
.dsh-atb-md h4 { font-size: 1em; }
.dsh-atb-md ul, .dsh-atb-md ol { margin: .45em 0 .7em; padding-left: 1.65em; }
.dsh-atb-md li { margin: .2em 0; word-break: break-word; }
.dsh-atb-md code {
  font-family: ui-monospace, SFMono-Regular, Menlo, Consolas, monospace; font-size: .92em;
  padding: .1em .32em; border-radius: 4px; word-break: break-word;
  color: var(--dsw-alias-label-primary, inherit);
  background: var(--dsw-alias-fill-tertiary, var(--dsw-bg-inset, rgba(128,128,128,.12)));
  border: 1px solid var(--dsw-alias-border-l2, rgba(128,128,128,.16));
}
.dsh-atb-md pre {
  display: block; box-sizing: border-box; width: auto; max-width: 100%; min-height: 0; height: auto;
  margin: .7em 0; padding: 9px 11px; overflow-x: auto; overflow-y: hidden; border-radius: 7px;
  white-space: pre; word-break: normal; text-align: left; font-size: .92em; line-height: 1.5;
  color: var(--dsw-alias-label-primary, inherit);
  background: var(--dsw-alias-fill-tertiary, var(--dsw-bg-inset, rgba(128,128,128,.12)));
...
.dsh-atb-md a:hover { text-decoration: underline; }
.dsh-atb-md blockquote {
  display: block; box-sizing: border-box; min-height: 0; height: auto;
  margin: .7em 0; padding: .25em 0 .25em 11px; border-left: 3px solid var(--dsw-alias-brand-primary, #8e4ec6);
  color: var(--dsw-alias-label-secondary, inherit); background: var(--dsw-alias-fill-tertiary, transparent);
  word-break: break-word; text-align: left;
}
.dsh-atb-md blockquote p:last-child { margin-bottom: 0; }
.dsh-atb-md-table-wrap {
  min-width: 0; max-width: 100%; margin: .7em 0; overflow-x: auto; overflow-y: hidden;
  -webkit-overflow-scrolling: touch;
}
.dsh-atb-md .dsh-atb-md-table-wrap > table {
  width: 100%; min-width: 640px; margin: 0; border-collapse: collapse; table-layout: auto;
  word-break: break-word; overflow-wrap: anywhere;
}
.dsh-atb-md th, .dsh-atb-md td {
  padding: 5px 8px; border: 1px solid var(--dsw-alias-border-l2, rgba(128,128,128,.28));
  text-align: left; vertical-align: top; word-break: break-word; overflow-wrap: anywhere;
}
.dsh-atb-md th {
  font-weight: 600; color: var(--dsw-alias-label-primary, inherit);
  background: var(--dsw-alias-fill-tertiary, var(--dsw-bg-inset, rgba(128,128,128,.1)));
}
.dsh-atb-md hr {
  height: 1px; margin: .9em 0; border: 0; background: var(--dsw-alias-border-l2, rgba(128,128,128,.3));
}
...
.dsh-atb-bubble-body { font-size: 12.5px; line-height: 1.55; word-break: break-word; }
...
.dsh-atb-newmenu-list {
  position: absolute; top: calc(100% + 4px); left: 0; z-index: 41; min-width: 220px;
  max-height: min(420px, calc(100vh - 100px)); overflow-y: auto;
...
.dsh-atb-newmenu-empty {
  display: flex; flex-direction: column; gap: 5px; padding: 10px; text-align: center;
...
.dsh-atb-tplm-meta {
  grid-area: meta; min-width: 0; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
  font-size: 10.5px; line-height: 1.35; color: var(--dsw-alias-label-tertiary, gray);
}
.dsh-atb-tplm-btns {
  grid-area: actions; display: flex; justify-content: flex-end; gap: 6px; flex-wrap: wrap;
}
...
.dsh-atb-set .dsh-atb-diag-sec { grid-column: 1 / -1; }
.dsh-atb-set .dsh-atb-mode-picker { margin-top: 8px; }
.dsh-atb-set .dsh-atb-template-category-select {
  width: 100%; box-sizing: border-box; margin-top: 8px; padding: 7px 9px;
  border-radius: 7px; border: 1px solid var(--dsw-alias-border-l2, rgba(128,128,128,.3));
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					...
					task.speed === "fast" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: "dsh-atb-badge",
						children: "⚡ 快速"
					}),
					task.permissionMode !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
						className: "dsh-atb-badge",
						title: "执行会话权限模式",
						children: task.permissionMode === "danger-full-access" ? "Full access" : task.permissionMode === "workspace-write" ? "Workspace Write" : "Read Only"
					}),
...
//#endregion
//#region src/client/file-paths.ts
...
function cleanReportedPath(raw) {
	let value = raw.trim();
...
function normalizePath(path) {
	...
	for (const segment of body.split("/")) {
		...
		if (segment === "..") {
			...
			if (previous !== void 0 && previous !== "..") segments.pop();
...
function taskFileBase(task, workspaces, execution, scope) {
	if (execution?.worktreePath !== void 0 && execution.worktreePath !== "") return execution.worktreePath;
	const workspace = workspaces.find((candidate) => candidate.id === task.workspaceId);
	if (workspace?.path !== void 0 && workspace.path !== "") return workspace.path;
	return scope?.repoRoot ?? scope?.cwd;
}
/** Resolve one report/git path and preflight it against the active session cwd. */
function resolveTaskFilePath(raw, task, workspaces, execution, scope) {
	const cleaned = cleanReportedPath(raw);
	if (cleaned === "" || cleaned.includes("\0")) return void 0;
	const base = taskFileBase(task, workspaces, execution, scope);
	const resolved = isAbsolutePath(cleaned) ? normalizePath(cleaned) : base === void 0 ? cleaned : joinPath(base, cleaned);
	if (!isAbsolutePath(resolved)) return {
		path: resolved,
		available: false,
		reason: "missing-base"
	};
...
//#endregion
//#region src/client/board/Avatar.tsx
/**
...
function getAvatarGradient(key, isUser = false) {
	if (isUser) return ["#64748b", "#475569"];
...
function getInitial(name, isUser = false) {
	if (isUser) return "U";
	if (!name) return "A";
	const match = name.match(/[a-zA-Z0-9]/);
	return match ? match[0].toUpperCase() : "A";
}
function InitialAvatar({ name, isUser = false, size = 26, className }) {
	const gradId = `atb-avt-${(0, react.useId)().replace(/[^a-zA-Z0-9_-]/g, "")}`;
...
function fenceInfo(rest, fence) {
	...
	if (fence[0] === "`" && info.includes("`")) return void 0;
	return info;
}
...
function hasMermaidFence(text) {
	...
	while (index < lines.length) {
		...
		const info = fenceInfo(line.slice(opening[0].length), fence);
		if (info === void 0 || !isMermaidInfo(info)) {
			const char = fence[0];
...
//#endregion
//#region src/client/mermaid-chunk-loader.ts
...
function asRuntime(value) {
	...
	return typeof candidate.initialize === "function" && typeof candidate.render === "function" ? candidate : void 0;
}
...
function injectChunkScript() {
	if (typeof document === "undefined") return Promise.reject(/* @__PURE__ */ new Error("Mermaid requires a browser document"));
	return new Promise((resolve, reject) => {
		...
		script.addEventListener("error", () => {
			script.remove();
			reject(/* @__PURE__ */ new Error(`Mermaid chunk failed to load: ${CHUNK_URL}`));
		}, { once: true });
...
function loadMermaid() {
	if (runtimeOverride !== void 0) return Promise.resolve(runtimeOverride);
	...
	inFlight = (async () => {
		...
		if (runtime === void 0) throw new Error("Mermaid chunk did not register a usable runtime");
		return runtime;
	})().catch((error) => {
		inFlight = void 0;
		throw error;
...
function sanitizeSvg(svg) {
	...
	document.querySelectorAll("*").forEach((node) => {
		...
		for (const attribute of [...node.attributes]) {
			...
			if (name.startsWith("on") || name.startsWith("@") || name === "href" || name === "xlink:href") node.removeAttribute(attribute.name);
		}
	});
...
function MermaidZoomModal({ svg, onClose }) {
	...
	const zoom = (0, react.useCallback)((delta, centerX, centerY) => {
		...
		const newScale = Math.min(8, Math.max(.2, current.scale * delta));
	...
	return (0, react_dom.createPortal)(/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		className: "dsh-atb-mermaid-modal",
		"data-mermaid-modal": true,
		ref: overlayRef,
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-atb-mermaid-modal-toolbar",
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
						type: "button",
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dsh-atb-mermaid-modal-stage",
				ref: stageRef
			}),
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dsh-atb-mermaid-modal-hint",
				children: "滚轮缩放 · 拖拽平移 · Esc 关闭"
			})
		]
	}), document.body);
}
...
function MermaidDiagram({ code }) {
	...
	const [zoomSvg, setZoomSvg] = (0, react.useState)(null);
	...
	const onBodyClick = (event) => {
		const svgEl = event.target.closest("svg") ?? event.currentTarget.querySelector("svg");
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		className: "dsh-atb-mermaid-wrap",
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				className: "dsh-atb-mermaid-head",
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
					className: "dsh-atb-mermaid-title",
					children: "mermaid"
				}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
					type: "button",
			...
			error !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dsh-atb-mermaid-error",
				"data-mermaid-state": "error",
				title: error,
				children: "Mermaid 渲染失败，已显示源码"
			}),
			svg !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
			...
			error !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("pre", {
				className: "dsh-atb-mermaid-code",
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", {
					className: "language-mermaid",
					children: rawDisplayCode
				})
			}),
			zoomSvg !== null && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(MermaidZoomModal, {
				svg: zoomSvg,
...
*/
function MermaidMarkdown({ html, className, onOpenFile }) {
	const containerRef = (0, react.useRef)(null);
	const mountsRef = (0, react.useRef)([]);
	(0, react.useLayoutEffect)(() => {
		const container = containerRef.current;
		if (container === null) return;
		const staleMounts = mountsRef.current.splice(0);
		if (staleMounts.length > 0) queueMicrotask(() => {
			for (const mount of staleMounts) mount.root.unmount();
		});
		...
		for (const pre of preElements) {
			...
			const root = (0, react_dom_client.createRoot)(mountEl);
			mountsRef.current.push({
				root,
				source
			});
			root.render(/* @__PURE__ */ (0, react_jsx_runtime.jsx)(MermaidDiagram, { code: source }));
		}
	}, [html]);
	(0, react.useEffect)(() => () => {
		const mounts = mountsRef.current.splice(0);
		if (mounts.length > 0) queueMicrotask(() => {
			for (const mount of mounts) mount.root.unmount();
		});
	}, []);
	const handleClick = (event) => {
		if (onOpenFile === void 0) return;
		const target = event.target;
		if (!(target instanceof Element)) return;
		const fileLink = target.closest("[data-dsh-atb-file]");
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		ref: containerRef,
		className,
		onClick: onOpenFile === void 0 ? void 0 : handleClick,
...
function normalizeMarkdown(text) {
	...
	for (let index = 0; index < lines.length;) {
		...
		if (line === line.trimStart() && SECTION_LABEL.test(line.trim())) {
			...
			if (next !== void 0 && LIST_ITEM.test(next.trimStart()) && next.trim().length > 0) output.push("");
			index += 1;
			continue;
		}
...
const defaultRenderer = new P();
/** Create a renderer for one Markdown pass and its optional file resolver. */
function createRenderer(resolveFileMention) {
	const renderer = new P();
	renderer.html = function({ text }) {
		return escapeHtml(text);
	};
	renderer.code = function({ text, lang }) {
		return `<pre><code${lang !== void 0 && lang.length > 0 ? ` class="language-${escapeHtml(lang)}"` : ""}>${escapeHtml(text)}\n</code></pre>\n`;
	};
	renderer.codespan = function({ text }) {
		const path = resolveFileMention?.(text);
		if (path !== void 0) return `<button type="button" class="dsh-atb-md-file-link" data-dsh-atb-file="${escapeHtml(path)}" title="${escapeHtml(path)}" aria-label="${escapeHtml(`打开文件 ${text}`)}">${escapeHtml(text)}</button>`;
	...
	renderer.link = function({ href, title, tokens }) {
		...
		if (title !== void 0 && title !== null) output += ` title="${escapeHtml(title)}"`;
	...
	renderer.image = function({ href, title, text, tokens }) {
		const alt = tokens !== void 0 ? this.parser.parseInline(tokens, this.parser.textRenderer) : text;
		const safe = safeHref(href);
		if (safe === void 0) return alt;
...
function renderMarkdown(text, options) {
	return f.parse(normalizeMarkdown(text), {
		gfm: true,
		breaks: true,
		async: false,
		renderer: options?.resolveFileMention === void 0 ? renderer : createRenderer(options.resolveFileMention)
	});
}
function Markdown({ text, className, resolveFileMention, onOpenFile }) {
	...
	if (hasMermaidFence(text)) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(MermaidMarkdown, {
		html,
		className: classes,
		onOpenFile
	});
...
function isLocalFileCandidate(raw, assumePath) {
	...
	return assumePath || isAbsolutePath(value) || value.includes("/") || value.includes(String.fromCharCode(92)) || /^[^:]+\.[^:]+$/.test(value);
}
/** Resolve an inline Markdown code token to a safe file opener target. */
function taskFileMentionResolver(task, workspaces, execution, scope, onOpenFile) {
	if (onOpenFile === void 0) return void 0;
	return (value) => {
		if (!isLocalFileCandidate(value, false)) return void 0;
		const target = resolveTaskFilePath(value, task, workspaces, execution, scope);
		return target?.available === true ? target.path : void 0;
	};
}
/** Render one report path as the shell's underlined open-file link. */
function OpenFileLink({ raw, target, onOpenFile }) {
	if (onOpenFile === void 0) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", {
		className: "dsh-atb-file-path",
		children: raw
	});
	if (target === void 0 || !target.available) {
		const reason = target?.reason === "outside-session-workspace" ? "当前会话 workspace 外，暂不可打开" : target?.reason === "missing-session-cwd" ? "当前会话路径尚未就绪" : "无法解析文件路径";
		return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
			className: "dsh-atb-file-unavailable-wrap",
			children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("code", {
				className: "dsh-atb-file-path",
				title: target?.path ?? reason,
				children: raw
			}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: "dsh-atb-file-unavailable",
				title: target?.path ?? reason,
				children: "不可访问"
			})]
		});
...
function FileOpenAction({ target, onOpenFile }) {
	if (onOpenFile === void 0) return null;
	if (target === void 0 || !target.available) {
		const reason = target?.reason === "outside-session-workspace" ? "当前会话 workspace 外，暂不可打开" : target?.reason === "missing-session-cwd" ? "当前会话路径尚未就绪" : "无法解析文件路径";
...
function ReportFileRow({ raw, task, execution, workspaces, scope, assumePath, onOpenFile }) {
	if (!isLocalFileCandidate(raw, assumePath)) return raw;
	return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
		className: "dsh-atb-file-row",
		children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(OpenFileLink, {
			raw,
			target: resolveTaskFilePath(raw, task, workspaces, execution, scope),
			onOpenFile
		})
	});
}
...
*/
function PreviewText({ text, className, raw, resolveFileMention, onOpenFile }) {
	if (raw) return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
		className: `${className} dsh-atb-raw`,
		children: text
	});
...
*/
function ReportBlock({ task, raw, workspaces, scope, onOpenFile }) {
	...
	const section = (label, rows, fileKind) => rows !== void 0 && rows.length > 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
			...
			children: rows.map((row, i) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("li", { children: fileKind !== void 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)(ReportFileRow, {
				raw: row,
				task,
				execution,
				workspaces,
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)(PreviewText, {
...
*/
function IsolationBlock({ task, controller, workspaces, scope, onOpenFile }) {
				...
				children: [
					...
					dirtyOpen && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [dirty.slice(0, 30).map((line, index) => {
							...
							return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: "dsh-atb-iso-dirty-row",
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
									type: "button",
								...
								}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(FileOpenAction, {
									target,
									onOpenFile
								})]
							}, `${line}-${index}`);
...
* @param now - current epoch ms (stale-claim highlight).
*/
function TaskDetail({ task, controller, now, fullScreen = false, onToggleFullScreen, scope, onOpenFile }) {
	const [comment, setComment] = (0, react.useState)("");
	const [showRawMarkdown, setShowRawMarkdown] = (0, react.useState)(false);
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [
			/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						className: "dsh-atb-detail-topbar",
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								...
								"data-active": showRawMarkdown ? "true" : void 0,
								...
								children: showRawMarkdown ? "渲染" : "原文"
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "dsh-atb-detail-edit",
							...
							onToggleFullScreen !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
								type: "button",
								className: "dsh-atb-detail-edit",
								"aria-pressed": fullScreen,
								title: fullScreen ? "退出详情全屏（Esc）" : "让详情占满当前看板区域",
								onClick: onToggleFullScreen,
								children: fullScreen ? "⛶ 退出全屏" : "⛶ 全屏"
							}),
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [
							...
							task.speed === "fast" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Chip, {
								icon: "⚡",
								children: "快速"
							}),
							task.permissionMode !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Chip, {
								icon: "🛡",
								children: task.permissionMode === "danger-full-access" ? "Full access" : task.permissionMode === "workspace-write" ? "Workspace Write" : "Read Only"
							}),
							...
							task.requiredCapabilities !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(Chip, {
								icon: "🔐",
								children: ["能力 · ", task.requiredCapabilities.join(", ")]
							}),
			...
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)(ReportBlock, {
				task,
				raw: showRawMarkdown,
				workspaces,
				scope,
				...
				children: [
					...
					task.comments.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						children: task.comments.map((c) => {
							const isAgent = c.threadId !== void 0;
							const modelName = isAgent ? task.model?.model : void 0;
							return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
								className: "dsh-atb-bubble",
								"data-from": isAgent ? "agent" : "user",
								children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: "dsh-atb-bubble-avatar",
									children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(InitialAvatar, {
										name: modelName,
										isUser: !isAgent,
										size: 26
									})
									...
									children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
										...
										children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("b", { children: isAgent ? `agent ${shortId(c.threadId)}` : "用户" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: fmtTime(c.createdAt) })]
									}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)(PreviewText, {
...
function supportsTaskFastSpeed(entry) {
...
function isTaskModelSupported(entry) {
...
function hasReasoningOptions(reasoning) {
	return reasoning !== void 0 && reasoning.efforts.length > 0;
}
/** Fast is usable only when the selected catalog row advertises priority. */
function speedForModel(model, speed) {
...
const GATE_CAPABILITY_LABELS = {
	browser: "浏览器",
	"computer-use": "电脑操作"
};
function gateCapabilityLabel(name) {
	return GATE_CAPABILITY_LABELS[name] ?? name;
}
function extraCapabilitiesOf(raw) {
	try {
...
function CapabilityEditor({ options, selected, onChange, permissionMode }) {
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		className: "dsh-atb-capabilities",
		children: [
			options.filter((option) => option.name !== TASKBOARD_CAPABILITY).filter((option, index, all) => all.findIndex((candidate) => candidate.name === option.name) === index).map((option) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("label", {
				className: "dsh-atb-capability",
				children: [
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("input", {
						type: "checkbox",
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: gateCapabilityLabel(option.name) }),
					/* @__PURE__ */ (0, react_jsx_runtime.jsx)("small", { children: option.name })
				]
			}, option.name)),
			selected.length > 0 && permissionMode === "danger-full-access" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: "dsh-atb-field-note dsh-atb-field-note-warn",
				children: "高权限文件访问与浏览器/电脑操作会一起授权，请确认任务来源可信。"
			}),
			/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
				className: "dsh-atb-field-note",
				children: "勾选项只在该任务执行或被会话认领时授权，不会改变其他会话。"
			})
		]
	});
}
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [
			rows.map((row, index) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				"data-editing": editing ? "true" : void 0,
				children: [
...
*/
function TaskFormModal({ controller, task, sessionId }) {
	...
	const [model, setModel] = (0, react.useState)(initialModel === void 0 ? "" : JSON.stringify(initialModel));
	const [speed, setSpeed] = (0, react.useState)(task?.speed === "fast" || prefill?.speed === "fast" ? "fast" : "standard");
	const [permissionMode, setPermissionMode] = (0, react.useState)(task?.permissionMode ?? (prefill?.permissionMode === "read-only" || prefill?.permissionMode === "danger-full-access" ? prefill.permissionMode : "workspace-write"));
	const [requiredCapabilities, setRequiredCapabilities] = (0, react.useState)(() => extraCapabilitiesOf(task?.requiredCapabilities ?? prefill?.requiredCapabilities));
	const [gateOptions, setGateOptions] = (0, react.useState)([]);
	const [gateDiscoveryReady, setGateDiscoveryReady] = (0, react.useState)(false);
	...
	(0, react.useEffect)(() => {
		...
		face().then((rows) => setCatalog(rows.filter(isTaskModelSupported))).catch(() => setCatalog([]));
	}, [controller]);
	(0, react.useEffect)(() => {
		...
		face().then((rows) => {
			if (cancelled) return;
			const names = rows.filter((row) => typeof row.name === "string" && row.name.length > 0).filter((row) => row.name !== TASKBOARD_CAPABILITY).filter((row, index, all) => all.findIndex((candidate) => candidate.name === row.name) === index);
	...
	(0, react.useEffect)(() => {
		...
		let selected;
		try {
			const value = JSON.parse(model);
			if (typeof value.provider === "string" && typeof value.model === "string") selected = {
				provider: value.provider,
				model: value.model
			};
		} catch {}
		if (!supportsTaskFastSpeed(selected === void 0 ? void 0 : catalog.find((row) => row.provider === selected?.provider && row.model === selected?.model))) setSpeed("standard");
	}, [catalog, model]);
	(0, react.useEffect)(() => {
	...
	*/
	...
	const selectedCatalog = selectedModel === void 0 ? void 0 : catalog.find((entry) => entry.provider === selectedModel.provider && entry.model === selectedModel.model);
	const reasoning = hasReasoningOptions(selectedCatalog?.reasoning) ? selectedCatalog?.reasoning : void 0;
	...
	const chooseModel = (value) => {
		...
		const picked = JSON.parse(value);
		const metadata = catalog.find((entry) => entry.provider === picked.provider && entry.model === picked.model);
		const next = {
			...picked,
			...metadata?.reasoning?.defaultEffort !== void 0 ? { reasoningEffort: metadata.reasoning.defaultEffort } : {}
		};
	...
	const submit = () => {
		...
		}, sessionId)).catch(() => void 0).finally(() => setBusy(false));
	};
		...
		(async () => {
			if (editing) {
				if (await controller.update(task.id, task.version, {
					...
					speed: effectiveSpeed,
					permissionMode,
					requiredCapabilities: normalizeRequiredCapabilities(["taskboard", ...requiredCapabilities]),
					...isolationOut !== void 0 && !isolationLocked ? { isolation: isolationOut } : {},
					presetId: presetOut ?? null,
					checklist: rows.length > 0 ? rows : null
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)(Field, {
							...
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
								value: selectedModel === void 0 ? "" : JSON.stringify({
									provider: selectedModel.provider,
									model: selectedModel.model
								}),
						...
						reasoning !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(Field, {
							label: "推理等级",
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
								value: effectiveEffort ?? "",
								onChange: (e) => chooseEffort(e.target.value),
								children: [reasoning.defaultEffort === void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", {
									value: "",
									children: "提供方默认"
								...
								className: "dsh-atb-field-note",
								children: "跟随模型目录提供的可用等级，不硬编码枚举"
							})]
						}),
						speedAvailable && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Field, {
							label: "速度模式",
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)("select", {
								value: effectiveSpeed,
								onChange: (e) => setSpeed(e.target.value === "fast" ? "fast" : "standard"),
								children: SPEED_OPTIONS.map((option) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("option", {
									value: option.value,
									children: [
										option.label,
										"（",
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)(Field, {
							...
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("select", {
								...
								children: PERMISSION_OPTIONS.map((option) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("option", {
									value: option.value,
									children: [
										option.label,
										"（",
							...
							}), permissionMode === "danger-full-access" && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "dsh-atb-field-note dsh-atb-field-note-warn",
								children: "Full access 将关闭审批并允许不受限制的文件修改，请确认任务可信。"
							})]
						}),
						...
						gateDiscoveryReady && gateOptions.length > 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)(Field, {
							label: "执行能力",
							full: true,
							children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(CapabilityEditor, {
								options: gateOptions,
...
//#endregion
//#region src/client/template-categories.ts
/** Compatibility label for templates created before categories existed. */
const DEFAULT_TEMPLATE_CATEGORY = "其他";
/** Resolve the display/filter category for one template. */
function templateCategoryOf(template) {
	const category = template.category?.trim();
	return category === void 0 || category.length === 0 ? DEFAULT_TEMPLATE_CATEGORY : category;
}
/** Derive stable, first-seen category options from the loaded template list. */
function templateCategoryOptions(templates) {
...
function matchesTemplateCategory(template, selected) {
	return selected === void 0 || templateCategoryOf(template) === selected;
}
...
function SettingsModal({ controller }) {
	const state = controller.getSnapshot();
	const currentIsolation = state.ledger.settings?.defaultIsolation ?? "none";
	const currentCategory = state.ledger.settings?.templateMenuCategory ?? "";
	const [draftIsolation, setDraftIsolation] = (0, react.useState)(currentIsolation);
	const [draftCategory, setDraftCategory] = (0, react.useState)(currentCategory);
	const categories = templateCategoryOptions(state.templates);
	const categoryOptions = currentCategory !== "" && !categories.some((o) => o.value === currentCategory) ? [...categories, {
					...
					children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("section", {
						...
						children: [
							...
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
								...
								children: [
									"当前保存的默认：",
									currentIsolation === "worktree" ? "🌿 Worktree 隔离" : "📁 原目录执行",
						...
						className: "dsh-atb-diag-sec",
						children: [
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h4", { children: "新建任务菜单的模板分类" }),
							/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("select", {
								...
								}), categoryOptions.map((option) => /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("option", {
									value: option.value,
									children: [
										option.value,
										"（",
										option.count,
										"）",
										option.count === 0 ? " · 当前无模板" : ""
									]
								}, option.value))]
							}),
							/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
								className: "dsh-atb-isolation-note",
								children: "只影响“+ 新建任务”菜单的模板展示，不会影响模板内容和已有任务；模板类别可在“管理模板”中调整。"
							})
						]
					})]
				}),
...
function TemplateManager({ controller }) {
	...
	const [categoryEdits, setCategoryEdits] = (0, react.useState)({});
	...
	const categoryOf = (id, fallback) => categoryEdits[id] ?? templateCategoryOf({ category: fallback });
	/** Save one template's rename and/or category. */
	const save = (id, name, category) => {
			...
			children: [
				/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							className: "dsh-atb-modal-headtext",
							children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("h3", { children: "任务模板" }), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("p", { children: "新建任务 ▼ 下拉的模板：改名 / 分类 / 删除 / 直接使用；任务详情页「存为模板」可新增" })]
						}),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							type: "button",
							className: "dsh-atb-modal-close",
				...
				/* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
					...
					children: state.templates.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						...
						className: "dsh-atb-empty2",
						children: "当前看板设置的类别暂无模板，请在看板设置中调整模板分类"
					}) : /* @__PURE__ */ (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("datalist", {
						id: "dsh-atb-template-categories",
						children: categories.map((option) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("option", { value: option.value }, option.value))
					}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
						className: "dsh-atb-tplm-list",
						children: visibleTemplates.map((t) => {
								...
								children: [
									...
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
										...
										children: [
											...
											t.task.permissionMode !== void 0 ? ` · ${t.task.permissionMode}` : ""
										]
									}),
									/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("span", {
...
*/
function TaskBoard({ controller, scope, visible, onOpenFile }) {
	const state = (0, react.useSyncExternalStore)((cb) => controller.subscribe(cb), () => controller.getSnapshot());
	const [now, setNow] = (0, react.useState)(() => Date.now());
	const [detailFullScreen, setDetailFullScreen] = (0, react.useState)(false);
	(0, react.useEffect)(() => {
	...
	(0, react.useEffect)(() => {
		if (selected === void 0) setDetailFullScreen(false);
	}, [selected?.id]);
	(0, react.useEffect)(() => {
	...
	const [collapsedColumns, setCollapsedColumns] = (0, react.useState)({
		backlog: true,
		todo: true,
		in_progress: false,
		in_review: false,
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
		...
		children: [
					...
					/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
							...
							children: [
								...
								visibleTemplates.length === 0 ? /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
									className: "dsh-atb-newmenu-empty",
									children: "当前分类暂无模板，请在看板设置中调整模板分类"
								}) : visibleTemplates.map((t) => /* @__PURE__ */ (0, react_jsx_runtime.jsx)("button", {
									type: "button",
									className: "dsh-atb-newmenu-opt",
					...
					return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
						...
						onDragOver: (e) => {
							if (e.dataTransfer.types.includes("application/x-dsh-atb-task")) {
								...
								if (isCollapsed) setCollapsedColumns((prev) => ({
									...prev,
									[status]: false
								}));
							}
						...
						children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
							...
							children: [
								...
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: COLUMN_LABELS[status] }),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: "dsh-atb-colcount",
									children: columnTasks.length
								}),
								/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
									className: "dsh-atb-coltoggle",
									"aria-hidden": "true",
									children: "▾"
								})
							]
			...
			selected !== void 0 && /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				...
				"data-fullscreen": detailFullScreen ? "true" : void 0,
			...
			return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
				...
				children: [/* @__PURE__ */ (0, react_jsx_runtime.jsxs)("div", {
					...
					children: [
						...
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: group.label }),
						/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
							className: "dsh-atb-colcount",
...
const TASKBOARD_TAB_ID = "dsh-taskboard:board";
...
*/
function registerBetterSidebarTab(ctx, controller) {
	...
	const descriptor = {
		...
		component: ({ ctx: sidebarCtx, scope, visible }) => {
			const liveService = getBetterSidebarService(sidebarCtx);
			const onOpenFile = liveService?.features.includes("openFile") === true ? (path) => {
				liveService.openFile(scope, path, fileTitle(path));
			} : void 0;
			return /* @__PURE__ */ (0, react_jsx_runtime.jsx)("div", {
				className: "dsh-atb-sidebar-tab",
				"data-dsh-atb-sidebar-tab": "",
				"data-visible": visible ? "true" : void 0,
				children: /* @__PURE__ */ (0, react_jsx_runtime.jsx)(TaskBoard, {
					controller,
					scope,
					visible,
					onOpenFile
	...
	const register = () => service.registerTab(descriptor);
	if (ctx.effect !== void 0) {
...
*/
function findTaskForSession(tasks, sessionId) {
	...
	for (const task of tasks) {
		...
		if (execution !== void 0 && (executionMatch === void 0 || execution > executionMatch.score || execution === executionMatch.score && task.updatedAt > executionMatch.task.updatedAt)) executionMatch = {
			task,
			score: execution
		};
		if (task.status === "in_progress" && task.claimedBy === sessionId && (claimMatch === void 0 || task.updatedAt > claimMatch.task.updatedAt)) claimMatch = {
			task,
...
//#endregion
//#region src/client/session-header-action.tsx
/**
...
*/
function TaskboardSessionHeaderAction({ sessionId, controller, onOpenTask }) {
	const task = findTaskForSession((0, react.useSyncExternalStore)((callback) => controller.subscribe(callback), () => controller.getSnapshot()).ledger.tasks, sessionId);
	...
	return /* @__PURE__ */ (0, react_jsx_runtime.jsxs)("button", {
		...
		children: [/* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", {
			className: "dsh-atb-session-link-icon",
			"aria-hidden": "true",
			children: "▦"
		}), /* @__PURE__ */ (0, react_jsx_runtime.jsx)("span", { children: "看板" })]
	});
}
/**
...
*/
function createTaskboardSessionHeaderAction(controller, onOpenTask) {
	return function TaskboardSessionHeaderSlot(props) {
		return /* @__PURE__ */ (0, react_jsx_runtime.jsx)(TaskboardSessionHeaderAction, {
...
function sessionIdFromCurrent(value) {
	if (typeof value === "string" && value.length > 0) return value;
	if (typeof value !== "object" || value === null) return void 0;
	const record = value;
	if (typeof record.id === "string" && record.id.length > 0) return record.id;
	return typeof record.sessionId === "string" && record.sessionId.length > 0 ? record.sessionId : void 0;
}
function currentInteractiveSessionId(ctx) {
...
function slotsFromContext(ctx) {
	try {
		const direct = ctx.slots;
		if (direct !== void 0 && typeof direct.inject === "function" && typeof direct.register === "function") return direct;
	} catch {}
	try {
		const fromService = ctx.get?.("slots");
		if (fromService !== void 0 && typeof fromService.inject === "function" && typeof fromService.register === "function") return fromService;
	} catch {}
}
/**
	...
	try {
		...
		if (connection !== void 0) {
			controller.installModelCatalog(async () => {
				const capabilityProvider = ctx.get?.(MODEL_CAPABILITY_SERVICE);
				const [response, capabilities] = await Promise.all([connection.api.llm.models({}), capabilityProvider?.listModelCapabilities().catch(() => []) ?? Promise.resolve([])]);
				if (!response.result.ok) return [];
				const capabilityMap = new Map(capabilities.map((capability) => [`${capability.provider}\u0000${capability.model}`, capability]));
				const out = [];
				for (const group of response.result.value.groups) for (const model of group.models) {
					...
					const row = {
						...
						...model.serviceTiers !== void 0 ? { serviceTiers: model.serviceTiers } : capability !== void 0 ? { serviceTiers: capability.serviceTiers } : {}
					};
			...
			let lazyGateDiscovery;
			const discoverLazyGate = (refresh = false) => {
				if (!refresh && lazyGateDiscovery !== void 0) return lazyGateDiscovery;
				const rpc = connection.rpc;
				lazyGateDiscovery = rpc?.call === void 0 ? Promise.resolve(void 0) : rpc.call("/tool-lazy-gate", "discover", {}).then((response) => {
					if (!response.ok || typeof response.value !== "object" || response.value === null) return void 0;
					const val = response.value;
					const skills = Array.isArray(val.enabledSkills) ? val.enabledSkills : Array.isArray(val.skills) ? val.skills : void 0;
					if (!Array.isArray(skills)) return void 0;
					return skills.filter((skill) => typeof skill === "object" && skill !== null && typeof skill.name === "string").map((skill) => ({ name: skill.name }));
				}).catch(() => void 0);
			...
			controller.installCurrentSessionAuthorizer(async (explicitSessionId) => {
				...
				if (skills === void 0 || !skills.some((skill) => skill.name === "taskboard")) return;
				const sessionId = explicitSessionId ?? currentInteractiveSessionId(ctx);
				if (sessionId === void 0) throw new Error("当前会话不可用，无法授权任务看板");
				const response = await connection.rpc?.call("/tool-lazy-gate", "grant-taskboard", { sessionId });
				if (response === void 0 || !response.ok) throw new Error("当前运行时不支持任务看板会话授权");
			});
		}
		...
		let registeredSidebarService;
		let registeredSidebarDisposer;
		let registeredHeaderSlots;
		let legacyDisposers = [];
		const disposeLegacyMounts = () => {
		...
		const openTaskFromHeader = (taskId, sessionId) => {
			const currentService = getBetterSidebarService(ctx);
			if (sidebarTabActive && currentService !== void 0 && registeredSidebarService === currentService && typeof currentService.openTab === "function") {
				if (currentService.isTabEnabled?.("dsh-taskboard:board") === false) return;
				const isTabActiveAndVisible = (typeof document !== "undefined" ? document.querySelector("[data-dsh-atb-sidebar-tab]") : null)?.getAttribute("data-visible") === "true";
				const rightPanel = typeof document !== "undefined" ? document.querySelector("[data-dsh-panel]") : null;
				const isPanelCollapsed = typeof document !== "undefined" && (document.body.hasAttribute("data-dsh-sidebar-collapsed") || rightPanel?.className.includes("panelHidden") === true);
				if (isTabActiveAndVisible && rightPanel !== null && !isPanelCollapsed) {
					const rightSidebarCollapseBtn = typeof document !== "undefined" ? document.querySelector("[data-dsh-panel-host] [class*=\"toggleCluster\"] button:last-child, [data-dsh-panel-host] button[aria-label*=\"侧边栏\"], [data-dsh-panel-host] button[aria-label*=\"側邊欄\"], [data-dsh-panel-host] button[aria-label*=\"sidebar\" i], [data-dsh-panel-host] button[aria-label*=\"折叠\"]") : null;
				...
				try {
					currentService.openTab?.({
						type: TASKBOARD_TAB_ID,
						path: "board"
					}, { sessionId });
				} catch (error) {
		...
		const registerHeaderAction = () => {
			...
			if (slots === void 0 || registeredHeaderSlots === slots) return;
			try {
		...
		const syncSidebarRegistration = () => {
			try {
				const currentService = getBetterSidebarService(ctx);
				if (currentService !== void 0) {
					if (registeredSidebarService === currentService) return;
					if (registeredSidebarDisposer !== void 0) {
						registeredSidebarDisposer();
						registeredSidebarDisposer = void 0;
					}
				...
				if (registeredSidebarService !== void 0) {
					if (registeredSidebarDisposer !== void 0) {
						registeredSidebarDisposer();
						registeredSidebarDisposer = void 0;
					}
					registeredSidebarService = void 0;
		...
		try {
			const unsubscribe = ctx.on?.("internal/status", () => {
				registerHeaderAction();
				syncSidebarRegistration();
			}, { global: true });
