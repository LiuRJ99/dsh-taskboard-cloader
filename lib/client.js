window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		function supportsTaskFastSpeed(model) {
			return model !== void 0 && /^gpt-5\.6(?:-|$)/i.test(model.trim());
		}
		...
		*/
		function TaskFormModal({ controller, task }) {
			...
			const [speed, setSpeed] = (0, react.useState)(supportsTaskFastSpeed(initialModel?.model) && (task?.speed === "fast" || prefill?.speed === "fast") ? "fast" : "standard");
