...
function requiredCapabilitiesOf(task) {
	return task.requiredCapabilities ?? ["taskboard"];
}
/**
...
const TASK_SPEEDS = ["standard", "fast"];
...
function asTaskSpeed(raw) {
	if (!TASK_SPEEDS.includes(raw)) throw new Error(`speed must be one of: ${TASK_SPEEDS.join(", ")}`);
	return raw;
}
...
function normalizeTemplateCategory(raw) {
	...
	if (category.length === 0) return void 0;
	if (category.length > 30) throw new Error(`template category must be 1..30 characters`);
	return category;
}
...
function asBoardSettings(raw) {
	...
	if (e.templateMenuCategory !== void 0) {
		const category = normalizeTemplateCategory(e.templateMenuCategory);
		if (category !== void 0) out.templateMenuCategory = category;
	}
...
function normalizeModel(raw) {
	if (typeof raw !== "object" || raw === null) throw new Error("model must be { provider: string, model: string, reasoningEffort?: string }");
	const { provider, model, reasoningEffort } = raw;
	if (typeof provider !== "string" || typeof model !== "string") throw new Error("model must be { provider: string, model: string, reasoningEffort?: string }");
	if (reasoningEffort !== void 0 && typeof reasoningEffort !== "string") throw new Error("model.reasoningEffort must be a string when provided");
...
function validateImportedTask(raw, now) {
	...
	try {
		...
		if (Array.isArray(e.comments)) for (const c of e.comments) {
			...
			const systemKey = typeof ce.systemKey === "string" && /^sys\.[A-Za-z0-9]+$/.test(ce.systemKey) && ce.systemKey.length <= 100 ? ce.systemKey : void 0;
			const systemParams = systemKey !== void 0 && typeof ce.systemParams === "object" && ce.systemParams !== null && !Array.isArray(ce.systemParams) ? Object.fromEntries(Object.entries(ce.systemParams).filter(([key, value]) => key.length <= 100 && typeof value === "string" && value.length <= 4e3).slice(0, 20)) : void 0;
			const systemRows = systemKey !== void 0 && Array.isArray(ce.systemRows) ? ce.systemRows.filter((row) => typeof row === "object" && row !== null && typeof row.repo === "string" && (row.repo === "" || isValidRelRepoPath(row.repo)) && [
				"merged",
				"noop",
				"failed"
			...
			comments.push({
				...
				...systemKey !== void 0 ? {
					systemKey,
					...systemParams !== void 0 ? { systemParams } : {},
					...systemRows !== void 0 ? { systemRows } : {}
				} : {}
			});
		}
...
function summarize(task) {
	...
	return {
		...
		model: task.model,
		...task.speed !== void 0 ? { speed: task.speed } : {},
...
//#endregion
export { ALL_STATUSES, DEFAULT_ISOLATION, DEFAULT_PERMISSION, MAIN_STATUSES, SECONDARY_STATUSES, TASKBOARD_CAPABILITY, TASK_SPEEDS, URGENCIES, approvalPolicyForPermissionMode, asBoardSettings, asIsolation, asPermission, asStatus, asTaskSpeed, asUrgency, canTransition, checklistFromTexts, checklistProgress, defaultIsolationOf, defaultPermissionOf, defaultSyncExternalSessionsOf, effectiveIsolation, effectivePrompt, effectiveTaskSpeed, emptyLedger, isClaim, isClaimedBy, isPlausibleTaskRecord, isValidRelRepoPath, isValidTaskId, newChecklistItemId, newCommentId, newExecutionId, newTaskId, nextCronTime, normalizeBody, normalizeBranchesMap, normalizeChecklist, normalizeChecklistText, normalizeExecution, normalizeExecutionReport, normalizeModel, normalizePrompt, normalizeRepoEvidence, normalizeRequiredCapabilities, normalizeTemplateCategory, normalizeTitle, parseCron, pruneExecutions, requiredCapabilitiesOf, summarize, syncClaim, taskAssociatedSessionIds, validateImportedTask, validateLedgerImport };
