...
function asPermissionMode(raw) {
	if (!PERMISSION_MODES.includes(raw)) throw new Error(`permissionMode must be one of: ${PERMISSION_MODES.join(", ")}`);
	return raw;
}
...
function summarize(task) {
	...
	return {
		...
		...task.permissionMode !== void 0 ? { permissionMode: task.permissionMode } : {},
...
//#endregion
export { ALL_STATUSES, DEFAULT_ISOLATION, MAIN_STATUSES, PERMISSION_MODES, SECONDARY_STATUSES, TASKBOARD_CAPABILITY, TASK_SPEEDS, URGENCIES, approvalPolicyForPermissionMode, asBoardSettings, asIsolation, asPermissionMode, asStatus, asTaskSpeed, asUrgency, canTransition, checklistFromTexts, checklistProgress, defaultIsolationOf, effectiveIsolation, effectivePrompt, effectiveTaskSpeed, emptyLedger, isClaim, isClaimedBy, isPlausibleTaskRecord, isValidTaskId, newChecklistItemId, newCommentId, newExecutionId, newTaskId, nextCronTime, normalizeBody, normalizeChecklist, normalizeChecklistText, normalizeExecution, normalizeExecutionReport, normalizeModel, normalizePrompt, normalizeRequiredCapabilities, normalizeTemplateCategory, normalizeTitle, parseCron, pruneExecutions, requiredCapabilitiesOf, summarize, syncClaim, validateImportedTask, validateLedgerImport };
